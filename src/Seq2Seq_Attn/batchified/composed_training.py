...
from Seq2Seq_Attn.batchified.train_com import train
from Seq2Seq_Attn.batchified.test_com import test
...
def trainIters(encoder, decoder, n_iters, training_pairs, test_pairs, infer_pairs, fnames, echk, dchk, input_vocab, output_vocab,
               rpath, use_cuda=False,print_every=1, plot_every=5,test_every=50,learning_rate=0.01, use_copy=True, use_attn=True,
               use_interim=False, train_attn = True, clip=0.25):
    start = time.time()
    ...
    plot_array = np.zeros((int(np.floor(n_iters/plot_every)), 5))
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % test_every == 0:
            ...
            for i, ip  in enumerate(infer_pairs):
                inferIters(encoder, decoder, ip, use_cuda, use_copy, use_attn, use_interim, train_attn, fnames[i])

        if iter % plot_every == 0:
            plot_array[idx,0] = iter
            plot_loss_avg = plot_loss_total / plot_every
            plot_acc_avg = plot_acc_total/ plot_every
            plot_array[idx,1] = plot_loss_avg
            plot_array[idx,2] = plot_acc_avg

            plot_test_l_avg = plot_test_loss / plot_every
            plot_test_a_avg = plot_test_acc / plot_every
            plot_array[idx,3] = plot_test_l_avg
            plot_array[idx,4] = plot_test_a_avg
