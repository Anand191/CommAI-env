...
for key,value in composed_train.items():
    ...
    for k, v in composed_train[key].items():
        ...
        data_com_tr[row,1] = all_targets(data_com_tr[row,0]) #k+ ' '+v
        row += 1
row = 0
for key,value in composed_test.items():
    ...
    for k, v in composed_test[key].items():
        ...
        data_com_te[row,1] = all_targets(data_com_te[row,0])#k+ ' '+v
        row += 1
...
for key,value in composed_infer.items():
    ...
    for k, v in composed_infer[key].items():
        ...
        data_com_if[row,1] = all_targets(data_com_if[row,0])#k+ ' '+v
        row += 1
...
df_te.to_csv('validation.csv',sep='\t',header=False,index=False)

df_if = pd.DataFrame(data_com_if)
df_if.to_csv('inference.csv',sep='\t',header=False,index=False)
