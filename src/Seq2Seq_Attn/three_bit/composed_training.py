...
from Seq2Seq_Attn.three_bit.train_com import train
from Seq2Seq_Attn.three_bit.test_com import test
from Seq2Seq_Attn.three_bit.checkpoint import checkpoint
from Seq2Seq_Attn.three_bit.infer_com import inferIters
...
def trainIters(encoder, decoder, n_iters, training_pairs, test_pairs, infer_pairs, fnames, use_cuda=False,
               print_every=1, plot_every=5,test_every=50,learning_rate=0.01, use_copy=True, use_attn=True,
               use_interim=False, clip=0.25):
    start = time.time()
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % test_every == 0:
            ...
            for i, ip  in enumerate(infer_pairs):
                inferIters(encoder, decoder, ip, use_cuda, use_copy, use_attn, use_interim,fnames[i])
########################################################################################################################
